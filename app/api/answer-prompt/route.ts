...
export async function POST(request: NextRequest) {
    try {
        ...
        const url = `${backendUrl}/api/answer-prompt`;

        const body = await request.json();

        // The frontend sends { journalPrompt, userInputs: { name, birthDate, ... }, readingId }
        // but the Python AnswerPromptRequest expects flat fields.
        const flatBody = {
            journalPrompt: body.journalPrompt,
            isCustom: body.isCustom ?? false,
            readingId: body.readingId,
            name: body.userInputs?.name ?? body.name,
            birthDate: body.userInputs?.birthDate ?? body.birthDate,
            birthCity: body.userInputs?.birthCity ?? body.birthCity,
            focusArea: body.userInputs?.focusArea ?? body.focusArea,
        };
